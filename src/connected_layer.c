...
void backward_connected_layer(layer l, matrix prev_delta)
{
	...
	axpy_matrix(1.0, matmul(transpose_matrix(l.w), delta), prev_delta);
    }
}
...
{
    // TODO
    // Subtract weight decay to get our weight updated
    axpy_matrix(-decay, l.w, l.dw);

    // scale weight update with learing rate and add it to our current weights
    axpy_matrix(rate, l.dw, l.w);

    // store momentum based weight change for the next weight update
